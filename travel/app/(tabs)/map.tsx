  ...
  const getAttractions = async (attractionsLocation: MapLocation) => {
    ...
    if (attractionsLocation) {
      ...
      try {
        const result = (
          await axios.post(
            `${API_URL}/attractions/nearby/${attractionsLocation.latitude}/${attractionsLocation.longitude}/400`
          )
        ).data;
        if (result) {
          const attractionMarkers = result.map(
            (attraction: {
